...
operations_return = Datasource(
    "operations_return",
    get_operations, depends_on=[
        wikitext.revision.datasources.diff.operations])
    ...
    while i < len(ops):
        ...
        if op.name == "delete":
            ...
            if len(ops) > i + 1 and ops[i + 1].name == "insert":
                ins_op = ops[i + 1]
                oprs_with_context.append(('replace',
                       [
               ...
               oprs_with_context.append(('delete',
                       [
                           ('context', "".join(_get_context_before(a, op.a1))),
                           ('delete', "".join(a[del_op.a1:del_op.a2])),
                           ('context', "".join(_get_context_after(a, op.a2)))
            ...
            oprs_with_context.append(('insert',
                   [
                       ('context', "".join(_get_context_before(a, op.a1))),
            ...
            oprs_with_context.append(('equal', [('context',
                "".join(a[op.a1:op.a2]))]))
        i += 1
    return oprs_with_context


def _get_context_before(tokens, pos):
    start = max(pos - CONTEXT, 0)
    for i in reversed(range(start, pos)):
        if "\n" in tokens[i]:
            start = i + 1
            break
...
def _get_context_after(tokens, pos):
    end = min(len(tokens) - 1, pos + CONTEXT)
...
operations_with_context = Datasource(
    "operations_with_context", process_operations_with_context,
    depends_on=[wikitext.revision.datasources.diff.operations])


relocation_segments_context = Datasource(
    "relocation_segments_context", process_relocation_context,
    depends_on=[wikitext.revision.datasources.diff.operations])
...
"""
operations_context = Datasource(
    "operations_context", process_operations_context,
    depends_on=[wikitext.revision.datasources.diff.operations])
...
segment added
"""
added_segments_context = Datasource(
    "added_segments_context", process_added_segments_context,
    depends_on=[wikitext.revision.datasources.diff.operations,
                operations_context])
...
segment removed.
"""
removed_segments_context = Datasource(
    "removed_segments_context", process_removed_segments_context,
    depends_on=[wikitext.revision.datasources.diff.operations,
                operations_context])
